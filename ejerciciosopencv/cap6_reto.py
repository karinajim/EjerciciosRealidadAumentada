...
ojo_izq_idx = [33, 133, 157, 158, 159, 160, 161, 173]
ojo_der_idx = [362, 263, 387, 388, 389, 390, 391, 398]
contador = 0
umbral_ear = 0.22
...
while True:
    ...
    if results.multi_face_landmarks:
        ...
        ear_izq, pts_izq = calcular_ear(landmarks, ojo_izq_idx, w, h)
        ear_der, pts_der = calcular_ear(landmarks, ojo_der_idx, w, h)
        ear_prom = (ear_izq + ear_der) / 2.0
        if ear_prom < umbral_ear and not parpadeo_activo:
            contador += 1
            parpadeo_activo = True
        elif ear_prom >= umbral_ear:
            parpadeo_activo = False
        for p in pts_izq + pts_der:
            cv2.circle(frame, p, 2, (0, 255, 255), -1)
        cv2.putText(frame, f"EAR: {ear_prom:.2f}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        cv2.putText(frame, f"Parpadeos: {contador}", (10, 60),
